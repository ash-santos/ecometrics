...
import com.example.carbonfootprint.databinding.HouseReportLayoutBinding
...
class HouseReport : AppCompatActivity() {

    lateinit var binding: HouseReportLayoutBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = HouseReportLayoutBinding.inflate(layoutInflater)
        ...
        binding.spinnerDate.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                user?.let {
                    ...
                    FirebaseQuery().readUserHouseEmission(it, year, month) { emission ->
                        FirebaseQuery().readDailyHouseEmissions(it, year, month) { daily ->
                            FirebaseQuery().readUserHouseYearlyEmissions(it, year) { monthly ->
                                FirebaseQuery().readYearlyHouseEmissions(
                                    it,
                                    year
                                ) { monthlyEmission ->
                                    ...
                                    if (position == 0) {
                                        ...
                                        emissionArray =
                                            FirebaseQuery().populateDailyHouseEmissionArray(daily)
                                        emissionData[0] = emission.electricityEmission.toString()
                                        emissionData[1] = emission.charcoalEmission.toString()
                                        emissionData[2] = emission.firewoodEmission.toString()
                                        emissionData[3] = emission.lpgEmission.toString()

                                    } else {
                                        //Yearly Emission Data
                                        emissionArray =
                                            FirebaseQuery().populateYearlyHouseEmissionArray(
                                                monthlyEmission
                                            )
                                        emissionData[0] = monthly.electricityEmission.toString()
                                        emissionData[1] = monthly.charcoalEmission.toString()
                                        emissionData[2] = monthly.firewoodEmission.toString()
                                        emissionData[3] = monthly.lpgEmission.toString()
                                    ...
                                    val array = arrayOf(
                                        arrayOf(
                                            ...
                                            getResources().getIdentifier("new_icon_electricity", "mipmap", getPackageName()).toString()
                                        ),
                                        arrayOf(
                                            "Charcoal",
                                            emissionData[1],
                                            MainActivity().percentageFormat((emissionData[1].toDouble() / dayEmission) * 100),
                                            getResources().getIdentifier("new_icon_charcoal", "mipmap", getPackageName()).toString()
                                        ),
                                        arrayOf(
                                            "Firewood",
                                            emissionData[2],
                                            MainActivity().percentageFormat((emissionData[2].toDouble() / dayEmission) * 100),
                                            getResources().getIdentifier("new_icon_firewood", "mipmap", getPackageName()).toString()
                                        ),
                                        arrayOf(
                                            "LPG",
                                            emissionData[3],
                                            MainActivity().percentageFormat((emissionData[3].toDouble() / dayEmission) * 100),
                                            getResources().getIdentifier("new_icon_lpg", "mipmap", getPackageName()).toString()
                                        )
                                    )
                                    ...
                                    val high = ContextCompat.getColor(this@HouseReport, com.example.carbonfootprint.R.color.high)
