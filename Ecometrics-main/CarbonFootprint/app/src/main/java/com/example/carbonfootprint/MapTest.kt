...
import com.example.carbonfootprint.databinding.MapTestBinding
...
class MapTest : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = MapTestBinding.inflate(layoutInflater)
        ...
        //addMarker(14.641684, 120.481842, "Marker 1");
        //addMarker(14.655684, 120.486842, "Marker 2");
        //addMarker(14.649684, 120.489842, "Marker 3");

        // Invalidate the map to refresh the display
        map.invalidate();

        // Example: Create a polygon overlay
        val polygonBalanga = listOf(
...
//hermosa
        ...
        /*insertPolygonToDatabase("Abucay", polygonAbucay)
        insertPolygonToDatabase("Bagac", polygonBagac)
        insertPolygonToDatabase("Balanga", polygonBalanga)
        insertPolygonToDatabase("Dinalupihan", polygonDinalupihan)
        insertPolygonToDatabase("Hermosa", polygonHermosa)
        insertPolygonToDatabase("Limay", polygonLimay)
        insertPolygonToDatabase("Mariveles", polygonMariveles)
        insertPolygonToDatabase("Morong", polygonMorong)
        insertPolygonToDatabase("Orani", polygonOrani)
        insertPolygonToDatabase("Orion", polygonOrion)
        insertPolygonToDatabase("Pilar", polygonPilar)
        insertPolygonToDatabase("Samal", polygonSamal)
        // Example usage:
        FirebaseQuery().retrievePolygonFromDatabase("Abucay") { geoPoints ->
            val polygonAbucay = geoPoints
            FirebaseQuery().retrievePolygonFromDatabase("Bagac"){ geo ->
                val polygonBagac= geo
                FirebaseQuery().retrievePolygonFromDatabase("Balanga"){
                    val polygonBalanga = it
                    val polygon = Polygon()
                    polygon.points = polygonBalanga
                    polygon.fillPaint.color = ContextCompat.getColor(this, R.color.redLight)
                    polygon.outlinePaint.color = ContextCompat.getColor(this, R.color.red)

                    val polygon2 = Polygon()
                    polygon2.points = polygonBagac
                    polygon2.fillPaint.color = ContextCompat.getColor(this, R.color.yellowLight)
                    polygon2.outlinePaint.color = ContextCompat.getColor(this, R.color.yellow)

                    val polygon3 = Polygon()
                    polygon3.points = polygonAbucay
                    polygon3.fillPaint.color = ContextCompat.getColor(this, R.color.yellowLight)
                    polygon3.outlinePaint.color = ContextCompat.getColor(this, R.color.yellow)

                    map.overlays.add(polygon)
                    map.overlays.add(polygon2)
                    map.overlays.add(polygon3)
    ...
    fun insertPolygonToDatabase(location: String, points: List<GeoPoint>) {
        val database = FirebaseDatabase.getInstance()
        val reference = database.getReference("polygons/$location")

        points.forEachIndexed { index, point ->
            reference.child("$index").setValue(mapOf("latitude" to point.latitude, "longitude" to point.longitude))
        }
    }
