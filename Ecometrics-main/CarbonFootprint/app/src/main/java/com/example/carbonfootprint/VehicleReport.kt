...
import com.example.carbonfootprint.databinding.VehicleReportLayoutBinding
...
class VehicleReport : AppCompatActivity() {
    ...
    lateinit var binding: VehicleReportLayoutBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = VehicleReportLayoutBinding.inflate(layoutInflater)
        ...
        binding.spinnerDate.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(
                ...
                user?.let {
                    ...
                    FirebaseQuery().readUserVehicleEmission(it, year, month) { emission ->
                        FirebaseQuery().readDailyVehicleEmissions(
                            it,
                            year,
                            month
                        ) { dailyEmission ->
                            FirebaseQuery().readUserVehicleYearlyEmissions(it, year) { monthly ->
                                FirebaseQuery().readYearlyVehicleEmissions(
                                    it,
                                    year
                                ) { monthlyEmission ->
                                    ...
                                    if (position == 0) {
                                        ...
                                        emissionArray =
                                            FirebaseQuery().populateDailyVehicleEmissionArray(
                                                dailyEmission
                                            )
                                        emissionData[0] = emission.premiumEmission.toString()
                                        emissionData[1] = emission.unleadedEmission.toString()
                                        emissionData[2] = emission.dieselEmission.toString()

                                    } else {
                                        //Yearly Emission Data
                                        emissionArray =
                                            FirebaseQuery().populateYearlyVehicleEmissionArray(
                                                monthlyEmission
                                            )
                                        emissionData[0] = monthly.premiumEmission.toString()
                                        emissionData[1] = monthly.unleadedEmission.toString()
                                        emissionData[2] = monthly.dieselEmission.toString()
                                    }

                                    val dayEmission = String.format(
                                        "%.2f",
                                        (emissionData[0].toDouble() + emissionData[1].toDouble() + emissionData[2].toDouble())
                                    ).toDouble()
                                    val array = arrayOf(
                                        arrayOf(
                                            "Premium",
                                            ...
                                            getResources().getIdentifier("new_icon_fuel", "mipmap", getPackageName()).toString()
                                        ),
                                        arrayOf(
                                            "Unleaded",
                                            emissionData[1],
                                    ...
                                    val high = ContextCompat.getColor(this@VehicleReport, com.example.carbonfootprint.R.color.high)

                                    val context = applicationContext

                                    val category = "Vehicle"
                                    ...
                                    emissionArray.forEachIndexed { index, data ->
                                        val totalEmission =
                                            data[1].toFloat() + data[2].toFloat() + data[3].toFloat()
